...
namespace OCS.UnitTests.BusinessLayer.Filters
{
    ...
    class CategoryFilterTests
    {
        ...
        public void Resolve_FiltersCorrectly()
        {
            ...
            var filterCategory = items[5].Category;
            var filter = new CategoryFilter(items, filterCategory.Name);

            var goodItems = new List<Product>();
            for (int i = 0; i < items.Count(); i++)
            {
                if (items[i].Category == filterCategory)
                {
            ...
            Assert.IsTrue(result.Filters.ContainsKey("Category"));
            var filteredItems = result.Filters["Category"] as IList<Product>;
        ...
        public void Resolve_StacksWithOtherFilters()
        {
            ...
            var filter = new CategoryFilter(new List<Product>(), "SomeFilter", secondFilter.Object);
